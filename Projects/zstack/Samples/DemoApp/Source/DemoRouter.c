...
static void sendLightButtonReport(void);
...
void zb_HandleKeys( uint8 shift, uint8 keys )
{
  ...
  else
  {
    ...
    if ( keys & HAL_KEY_SW_2 )
    {
      //Send ligt button report when pressed on button 2
      sendLightButtonReport();
    }
    if ( keys & HAL_KEY_SW_3 )
    {
    }
...
void zb_BindConfirm( uint16 commandId, uint8 status )
{
  if( status == ZB_SUCCESS )
  {
    ...
    // Zet na de BindConfirm hem weer in AllowBind
    zb_AllowBind( 0xFF );
...
void zb_ReceiveDataIndication( uint16 source, uint16 command, uint16 len, uint8 *pData  )
{
  ...
  // If recieved LIGHT_REPORT_CMD_ID switch the led state and turn it on or off
...
static void sendLightButtonReport(void)
{
  zb_SendDataRequest( 0xFFFE, LIGHT_BUTTON_CMD_ID, NULL, NULL, 0, 0, 0 );
}
/******************************************************************************
 * @fn          uartRxCB
 *
